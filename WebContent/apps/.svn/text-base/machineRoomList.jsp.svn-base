...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function add() {
			location.href = "${ctx}/machineRoom_input.do";
		}
		function edit() {
			...
			location.href = "${ctx}/machineRoom_input.do?id="+id;
		}
		function del() {
			...
			$("#mainForm")[0].action="${ctx}/machineRoom_delete.do";
			$("#mainForm").submit();
		}
		function showIndex() {
			var len = $("input[type=checkbox][name=ids]:checked").length;
			...
			var id = $("input[type=checkbox][name=ids]:checked").val();
			location.href = "${ctx}/machineRoomIndex.do?roomId="+id;
		}
		</script>
	</head>
